...
data-dialog-type="modal" href="requestSupport"><img src="' . $support_image_path . '/mo-customer-support.png" alt="support image"></a>'),
  ...
  /**
   * @param $ldapconn \LDAP\Connection object
   *
   * @return  string error message,error code and diagnostic error if any
   */
  public static function getLDAPDiagnosticError($ldapconn): string {

    if(!$ldapconn){
      return '';
    }

    $errors = 'Error : '.ldap_error($ldapconn).' ('.ldap_errno($ldapconn).') <br>';
    if( ldap_get_option($ldapconn, LDAP_OPT_DIAGNOSTIC_MESSAGE,$diagnostic_message)){
      $errors .= 'Error Description : '.$diagnostic_message.'<br>';
    }
    return $errors;
  }
