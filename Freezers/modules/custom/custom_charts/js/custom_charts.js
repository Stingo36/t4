      ...
      $(document).ready(function () {
        ...
        $('#buttonTrigger').html(`
          <div class="d-flex flex-wrap justify-content-center">
            <button id="show-graph" class="btn custom-btn mx-2 my-1">Show Graph</button>
            <button id="show-data" class="btn custom-btn mx-2 my-1">Show Data</button>
            <button id="show-details" class="btn custom-btn mx-2 my-1">Show Details</button>
          </div>
        `);
        ...
        // Function to display the "No Data Found" message
        function showNoDataMessage() {
          $("#chartdiv").hide(); // Hide the chart container
          $("#freezer-title").hide(); // Hide the title container
          $('#buttonTrigger, #freezer-data-container, #details-container').hide(); // Hide buttons, data, and details container
          toggleFilterButtons(false); // Hide filter buttons

          // Display the "No Data Found" message
          $("#Status").html('<h2 class="text-center">No Data Found</h2>');
        }
          ...
          fetchData(url, function (data) {
            ...
            try {
              am5.ready(function () {
                ...
                var chartHeight = 450; // Set the desired height in pixels
        ...
        function loadFreezerData(freezerName, page) {
          ...
          fetchData(url, function (data) {
            ...
            var table = $('<table></table>').addClass('table').css('margin-top', '20px');
            ...
            if (totalPages > 1) {
              ...
              var ul = $('<ul></ul>').addClass('pagination justify-content-center no-border');
...
function displayDetails() {
  ...
  // Create the initial HTML content for the table
  var detailsHtml = `
    <div class="text-center">
      <table class="table table-borderless table-sm">
        <tbody>
          <tr><td><strong>Freezer Name:</strong></td><td>${freezerName}</td></tr>
          <tr><td><strong>Location:</strong></td><td>${location}</td></tr>
          <tr><td><strong>Floor:</strong></td><td>${floor}</td></tr>
          <tr style="background-color: #333; color: white;"><td colspan="2" class="text-center"><strong>Status: Loading...</strong></td></tr>
        </tbody>
      </table>
    </div>
  `;

  // Set the initial HTML content to the details container
  $('#details-container').html(detailsHtml);

  if (freezerName !== 'Not selected') {
    // Make an AJAX request to the controller to get the freezer status
    $.ajax({
      url: `freezer/check-status/${freezerName}`,
      type: 'GET',
      dataType: 'json',
      success: function (response) {
        // Check if the response contains any errors
        if (response.error) {
          $('#freezer-status').text('Error: ' + response.error);
        } else {
          // Prepare the faculties display
          var facultiesDisplay = 'N/A';
          if (Array.isArray(response.faculties) && response.faculties.length > 0) {
            facultiesDisplay = response.faculties.map(faculty => faculty.name).join(', ');
          }

          // Save values to variables for reuse
          var status = response.status || 'N/A';
          var currentTemperature = response.current_value || 'N/A';
          var currentTime = response.current_time || 'N/A';
          var allTimeHighestTemperature = response.all_time_highest_temperatu || 'N/A';
          var allTimeHighTime = response.all_time_high_time || 'N/A';
          var allTimeLowestTemperature = response.all_time_lowest_temperatu || 'N/A';
          var allTimeLowTime = response.all_time_low_time || 'N/A';
          var maximumThreshold = response.maximum_threshold || 'N/A';
          var minimumThreshold = response.minimum_threshold || 'N/A';

          // Construct the combined table content
          var combinedTableHtml = `
            <div class="text-center">
              <table class="table table-borderless table-sm">
                <tbody>
                  <tr style="background-color: #333; color: white;"><th colspan="2" class="text-center">Freezer Details</th></tr>
                  <tr><td><strong>Freezer Name:</strong></td><td>${freezerName}</td></tr>
                  <tr><td><strong>Location:</strong></td><td>${location}</td></tr>
                  <tr><td><strong>Floor:</strong></td><td>${floor}</td></tr>
                  <tr><td><strong>Status:</strong></td><td>${status}</td></tr>
                  <tr><td><strong>Faculties:</strong></td><td>${facultiesDisplay}</td></tr>
                  <tr style="background-color: #333; color: white;"><th colspan="2" class="text-center">Temperature Status</th></tr>
                  <tr><td><strong>Current Temperature:</strong></td><td>${currentTemperature} at ${currentTime}</td></tr>
                  <tr><td><strong>All Time Highest Temperature:</strong></td><td>${allTimeHighestTemperature} at ${allTimeHighTime}</td></tr>
                  <tr><td><strong>All Time Lowest Temperature:</strong></td><td>${allTimeLowestTemperature} at ${allTimeLowTime}</td></tr>
                  <tr style="background-color: #333; color: white;"><th colspan="2" class="text-center">Threshold Values</th></tr>
                  <tr><td><strong>Maximum Threshold:</strong></td><td>${maximumThreshold}</td></tr>
                  <tr><td><strong>Minimum Threshold:</strong></td><td>${minimumThreshold}</td></tr>
          `;

          // If there is a download link, add it to the table
          if (response.download_data) {
            combinedTableHtml += `<tr><td colspan="2" class="text-center"><a href="${response.download_data}" download>Download All Data</a></td></tr>`;
          }

          combinedTableHtml += `</tbody></table></div>`;

          // Update the details container with the constructed message
          $('#details-container').html(combinedTableHtml);
        }
      },
      error: function () {
        $('#freezer-status').text('Error: Unable to fetch the freezer status.');
      }
    });
  } else {
    $('#freezer-status').text('No freezer selected.');
  }
}
