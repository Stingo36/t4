...
class FreezerDataController extends ControllerBase
{
    ...
    protected function updateContentWithLogFile($log_file_path)
    {
        // Check if the content type "logs" exists
        $node_storage = \Drupal::entityTypeManager()->getStorage('node');
        $query = $node_storage->getQuery();
        $query->condition('type', 'logs') ->condition('title', 'Response Logs')->accessCheck(FALSE);
        $query->range(0, 1);
        $nids = $query->execute();
    ...
    private function createOrUpdateNode($data, $term, $formatted_time)
    {
        ...
        if (empty($existing_nodes)) {
            $node = \Drupal\node\Entity\Node::create([
                ...
                'field_all_time_highest_temperatu' => $data['value'], // Initialize with the current value
                'field_all_time_lowest_temperatur' => $data['value'],  // Initialize with the current value
                'field_all_time_high_time' => $formatted_time,            // Initialize with the current time
                'field_all_time_low_time' => $formatted_time,             // Initialize with the current time
            ]);
        } else {
            $existing_node_id = reset($existing_nodes);
            $node = \Drupal\node\Entity\Node::load($existing_node_id);
    
            // Update the 'field_current_value' and 'field_current_time' with the new value
        ...
        // Save the CSV file to the field_download_data field
        ...
        // Attach the file to the node's field_download_data
        $node->set('field_download_data', [
            'target_id' => $file->id(),
            'display' => 1,
          ]);
    
        // Read the CSV file to find the highest and lowest values and their times
        $file = fopen($file_name, 'r');
        $values = [];
        $timestamps = [];
    
        while (($line = fgetcsv($file)) !== FALSE) {
            $values[] = (float)$line[0];          // Assuming the first column is the value
            $timestamps[] = $line[1];             // Assuming the second column is the time
        }
    
        fclose($file);
    
        if (!empty($values)) {
            // Find the highest and lowest values and their corresponding times
            $highest_index = array_keys($values, max($values))[0];
            $lowest_index = array_keys($values, min($values))[0];
    
            $highest_value = $values[$highest_index];
            $lowest_value = $values[$lowest_index];
            $highest_time = $timestamps[$highest_index];
            $lowest_time = $timestamps[$lowest_index];
    
            // Update the fields with the highest and lowest values and their corresponding times
            $node->set('field_all_time_highest_temperatu', $highest_value);
            $node->set('field_all_time_lowest_temperatur', $lowest_value);
            $node->set('field_all_time_high_time', $highest_time);
            $node->set('field_all_time_low_time', $lowest_time);
        }
