 ...
 *
 * @ViewsField("twilio_user_number")
 */
class TwilioUserPhone extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    $phone_no = $this->getTwilioPhone($uid);
   ...
   */
  private function getTwilioPhone($uid) {
    /** @var \Drupal\Core\Database\Connection $connection */
    $connection = \Drupal::service('database');

    $phone_no = $connection->select('twilio_user')
      ->fields('twilio_user', ['number'])
      ->condition('uid', $uid)
      ->execute()
      ->fetchField();
