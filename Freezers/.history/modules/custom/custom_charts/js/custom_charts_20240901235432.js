...
function displayDetails() {
  ...
  // Create initial HTML content to display the details
  var detailsHtml = `
    <div class="text-center">
      ...
      <p id="freezer-status">Loading status...</p>
    </div>
  `;
  ...
  // Check if a freezer name is selected
  if (freezerName !== 'Not selected') {
    // Make an AJAX request to the controller to get the freezer status
    $.ajax({
      url: `/freezer/check-status/${encodeURIComponent(freezerName)}`,
      type: 'GET',
      dataType: 'json',
      success: function (response) {
        // Update the status in the details container
        $('#freezer-status').text('Freezer Status: ' + response.status);
      },
      error: function () {
